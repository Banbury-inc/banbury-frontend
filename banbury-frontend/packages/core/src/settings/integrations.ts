...
/**
 * Get Google Calendar integration status using existing Google Drive credentials
 * Google Calendar uses the same Google API credentials as Google Drive
 */
 ...
 */
export const enableGoogleCalendarIntegration = async (): Promise<{ 
  result: string; 
  message?: string;
}> => {
  try {
    ...
    if (!calendarAccess.hasAccess) {
      if (calendarAccess.message.includes('scope not granted')) {
        return {
          result: 'error',
          message: 'Google Calendar scope missing from your Google credentials. Please disable and re-enable Google Drive integration to get Google Calendar permissions.'
        };
      } else {
        return {
          result: 'error',
